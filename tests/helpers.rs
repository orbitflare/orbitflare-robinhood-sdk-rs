...
use orbitflare_robinhood_sdk::error::Error;
...
fn with_endpoint_prepends_host_on_wrapping_variants() {
    ...
    for e in cases {
        let e = e.with_endpoint("https://robinhood.rpc.orbitflare.com");
        let msg = format!("{e}");
        assert!(
            msg.contains("[https://robinhood.rpc.orbitflare.com]"),
...
fn with_endpoint_strips_api_key_from_host() {
    ...
    let e = e.with_endpoint("https://robinhood.rpc.orbitflare.com?api_key=ORBIT-SECRET-123");
    let msg = format!("{e}");
    assert!(!msg.contains("ORBIT-SECRET-123"), "leaked api_key: {msg}");
    assert!(msg.contains("robinhood.rpc.orbitflare.com"));
}
